...
## 6. 负面结果：为什么训练后，有些能力反而变差？

训练并没有让所有任务一起变好，而且“性能下降”至少包含三种不同现象：

- **原有能力退化**：Prompt Injection 的准确率从原始 MDLM 的 82.76% 降到 S1 的 70.69%，概率质量也变差。
- **准确率恢复，但校准没有恢复**：Emotion 在 S0 时为 53.80%，低于原始模型的 55.65%；S1 恢复到 56.35%，但 ECE 从原始模型的 7.95% 上升到 23.02%。这里不能简单说 S1 的分类准确率退化了。
- **增加数据没有延续已有收益**：AG News 从原始模型的 79.07% 提升到 S0 的 82.55%，扩充为 S1 后却回落到 80.91%。它仍高于原始模型，但没有从这次扩充中继续获益。

从 S0 到 S1，六套任务的等权平均准确率也只从 60.48% 变为 60.54%。这提醒我们，需要解释的是**哪些能力被强化、哪些判断边界被改变，以及置信度为什么与正确率脱节**，而不只是训练样本是否足够多。

### 6.1 相同的 Yes/No 接口，不代表相同的判断任务

Prompt Injection 的错误变化很有代表性。完整测试集中有 60 条注入、56 条非注入：

| 模型 | 检出的注入 / 60 | 正确识别的非注入 / 56 |
|---|---:|---:|
| 原始 MDLM | 44 | 52 |
| S0 | 27 | 56 |
| S1 | 26 | 56 |

训练后的模型几乎不再误报，但漏报明显增加。它不是随机地变差，而是判断边界偏向了“不是注入”。

一个直观解释是训练集中 No 更多，但实际并非如此：S1 的硬标签 noul 中，Yes 为 4,339 条，No 为 4,069 条，基本平衡。因此，单纯的标签频率失衡不足以解释这个结果。

更值得关注的是监督的语义。SNLI 的二元任务主要问“前提是否蕴含命题”，BoolQ 问“给定 passage 后问题的答案是否成立”，程序题则检查显式条件。它们强化的是**基于材料寻找支持或执行规则**。注入检测要求识别文本是否试图改变系统行为、冒充角色或越过指令边界，需要的判断依据并不一样。

因此，一个合理但尚未通过消融验证的解释是：训练强化了某种支持性、较保守的 Yes/No 判断，却没有提供足够的独立监督来保持指令边界识别。这里检测的是输入是否属于注入，不是生成模型是否实际服从了攻击，不能把二者等同。

这暴露了统一接口的重要边界：**输出格式可以统一，所需的能力覆盖仍必须分别考虑。**

### 6.2 数据量增加了，关键能力的有效监督未必增加很多

GoEmotions 是 S1 中直接补充情绪能力的来源，但 3,000 条并不等于 3,000 条均衡的目标情绪监督。实际采样中的部分标签如下：

| GoEmotions 标签 | 新增训练条数 |
|---|---:|
| neutral | 1,084 |
| love | 119 |
| anger | 94 |
...
neutral 占 36.1%；与当前 Emotion 六类同名的标签合计只有 442 条。其他细粒度标签当然也可能帮助迁移，但不能把整套数据都理解为直接覆盖这六个边界。再加上 Reddit 评论与目标文本的风格差异、两套标签体系的粒度差异，增加一个“相关来源”并不保证所有情绪类别都获得同等收益。

AG News 面临的是另一种差异。DBpedia 训练“实体描述属于哪种本体类别”，新闻分类则需要根据事件和语境判断主题。公司、运动员或作品的身份可能提供线索，但并不直接决定新闻主题。S1 在新增 DBpedia 诊断 dev 上达到 99.75% 准确率，AG News 却比 S0 下降，说明**来源内学得很好，不等于迁移任务继续变好**。是否更依赖实体线索，需要进一步的错误分析和来源消融才能确认。

此外，我们使用全参数训练，所有任务会更新同一个 backbone 和共享读出。新来源不仅补充知识，也可能重新分配模型对不同线索的重视程度。当前目标没有显式要求保留原始模型在未覆盖任务上的行为，因此观察到的变化与任务干扰、能力保持不足相容。不过，还不能据此认定某一个数据源是退化的唯一原因。

### 6.3 为什么准确率提高，ECE 却可能更差？

准确率只取决于谁的分数最高，ECE 还取决于模型对这个选择有多自信。二者并不要求同步变化。

一个简单例子是将候选分数统一放大：

$$
p_i^{(\alpha)}=\frac{\exp(\alpha s_i)}{\sum_j\exp(\alpha s_j)},\qquad \alpha>1.
$$

在没有并列最高分的情况下，argmax 不变，但分布变得更尖锐。如果原本就判断错了，模型也会对错误更加自信。即使模型通过训练修正了一部分答案，只要剩余错误的置信度上升得更快，ECE 或 NLL 仍可能恶化。这里的缩放只是解释机制的示意，不意味着实际训练只改变了一个全局温度。

交叉熵为什么没有阻止这种现象？**交叉熵本身就是概率评分目标，不是只优化 argmax 的损失。** 在理想的同分布、充分数据和合适模型条件下，它有利于学习真实条件概率。但在有限样本上，硬标签训练会持续奖励正确类别的高概率；模型可以很好地拟合训练来源，同时在未覆盖的文本风格或判断任务上给出过大的分数间隔。训练分布上的目标下降，并不保证域外校准同步改善。

本实验中的全参数更新、数据分布差异和不均衡的能力覆盖，都可能参与这个过程。对 Shared Yes/No 而言，choice 的概率来自候选间的相对 log-odds。给所有候选加同一个 Yes/No 常数偏置会在 softmax 中抵消，因此不能把所有校准退化都解释为“模型更喜欢输出 Yes”；候选表示及其相对分数间隔的变化同样重要。

已有结果也支持“答案选择与置信度分离”这一观察：在额外检查的 S1 epoch 2 末 checkpoint（step 2,500）中，三套外部任务准确率提高、三套下降，但六套任务的 ECE、NLL 和 Brier 都比 step 1,300 更差。继续训练并没有普遍改善外部概率质量。这是后续诊断，主结果仍使用原 dev 规则选出的 step 1,300。

### 6.4 为什么 dev 选优仍不能完全避免这些问题？

为了让 S0/S1 对比主要反映训练数据变化，我们保留了原 S0 的 1,000 条 dev，并按 mean KL 选 checkpoint。这个规则避免了使用外部测试挑模型，也确实选中了概率质量优于较晚 checkpoint 的权重。

但原 dev 主要覆盖 S0 的来源与任务家族，不是所有迁移能力的充分代理。模型可以改善这些任务的 KL，同时损失未被 dev 充分覆盖的注入判断或情绪校准。**选模规则正确执行，不代表开发集已经覆盖了所有重要能力。**

下一步需要用实验区分这些解释：来源消融检查任务干扰；匹配总训练曝光的 S0-long 区分“更多数据”与“训练更久”；来源隔离 dev 检查选模与迁移目标的关系；在独立 dev 上拟合温度，则可判断多少概率误差来自分数尺度。温度缩放通常不改变 argmax，无法单独修复注入检测的漏报；测试集仍只用于最终评估。

因此，负面结果不是在说统一读出不可行，而是在指出它下一阶段需要解决的两件事：**新监督能否覆盖正确的判断能力，以及适配后的分数还能否作为可靠概率使用。**

## 7. Future Work

### 7.1 面向校准的训练 loss

决策模型最终交给程序的是概率，因此下一阶段会进一步研究**不同概率评分目标与分布保持项对跨来源校准的影响**。当前 choice / noul 已使用交叉熵这一 log score，score 额外使用 RPS；未来可以系统比较 log score、Brier score、RPS 以及轻量的分布保持项，让“选对答案”和“给出合适置信度”在同一个训练目标里协同优化。
